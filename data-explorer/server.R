...
shinyServer(function(input, output) {
    ...
    df_highlight <- reactive({
        if (input$district != "") {
            df[df$system_name == input$district, ]$state <- 2
            df[df$system_name == input$district, ]$opac <- 1
        }
    ...
    # Extract district number of hovered point for secondary graphs, set default to State (district = 0)
    ...
    plot2 <- reactive({
        ...
        district_data %>%
            ggvis(~Percentage, ~factor(demographic)) %>%

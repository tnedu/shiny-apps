...
determinations <- read_csv("data/final_determinations.csv") %>%
    select(system_name, minimum_performance_goal, achievement_determination, 
    ...
    mutate(minimum_performance_goal = ifelse(is.na(minimum_performance_goal), "N/A", minimum_performance_goal),
        achievement_determination = ifelse(is.na(achievement_determination), "N/A", achievement_determination),
        gap_determination = ifelse(is.na(gap_determination), "N/A", gap_determination),
        final_determination = ifelse(is.na(final_determination), "N/A", final_determination))

district_list <- unique(determinations$system_name)

color_list <- c("Met" = "blue", "Missed" = "red", "N/A" = "black", 
    "In Need of Improvement" = "red", "Progressing" = "yellow", "Achieving" = "green", "Exemplary" = "blue")

# District achievement, profile data for comparison tool
ach_profile <- read.csv("data/achievement_profile_data_with_CORE.csv", stringsAsFactors = FALSE)
...
df_outcomes <- ach_profile %>%
    select(system_name, Math, ELA, Science, AlgI, AlgII, BioI, Chemistry, EngI, EngII, EngIII,
        Graduation, Dropout, ACT_Composite, Pct_Chronically_Absent, Pct_Suspended, Pct_Expelled)